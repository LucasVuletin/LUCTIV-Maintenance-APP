...
import { createOperationalCapture } from '../src/app/prime/capture-logic';
import { mapFailureSummary } from '../src/app/prime/export-mapper';
import { findFailureRule, requiresTechnicalValidation } from '../src/app/prime/failure-rules';
import { PRIME_HEADERS } from '../src/app/prime/schema';
import { validateFailureCase } from '../src/app/prime/validation';

function failureInput(pumpId = '6672'): FailureCaseInput {
  return { affectedPumpId: pumpId, detectionSource: 'Manual - Field', failureArea: 'Fluid end', failureReason: 'Empaque', failureEvidence: 'Evidencia física', diagnosisStatus: 'Suspected', responsibleGroup: 'PE', conditionClass: 'Almost / Consumable', plannedAction: 'Repair In-Line', minutesToRecovery: 12, taskDescription: 'Inspeccionar empaque', ruleId: 'RULE-002', ruleStatus: 'Source example' };
}

test('reuses the open CaseId', () => {
  const state = createPrimeDemoState();
  const first = createOrUpdateFailureCase([], state.stage, failureInput(), 1, '2026-08-06T10:00:00.000Z');
  const repeated = createOrUpdateFailureCase([first], state.stage, { ...failureInput(), failureEvidence: 'Nueva evidencia' }, 1, '2026-08-06T10:05:00.000Z');
...
test('creates a new CaseId after closure', () => {
  const state = createPrimeDemoState();
  const first = createOrUpdateFailureCase([], state.stage, failureInput(), 1, '2026-08-06T10:00:00.000Z');
  const next = createOrUpdateFailureCase([{ ...first, closedAt: '2026-08-06T10:30:00.000Z' }], state.stage, failureInput(), 2, '2026-08-06T11:00:00.000Z');
  assert.equal(next.caseId, 'CASE-6672-20260806-02');
});

test('validates PartOfPlan and DeferredReason', () => {
  const state = createPrimeDemoState();
  const base = createOrUpdateFailureCase([], state.stage, failureInput(), 1, '2026-08-06T10:00:00.000Z');
  assert.ok(validateFailureCase({ ...base, partOfPlan: 'Yes', sttOrder: null }, state.pumps).sttOrder);
  assert.ok(validateFailureCase({ ...base, partOfPlan: 'No', deferredReason: '', workStatus: 'Backlog' }, state.pumps).deferredReason);
});

test('requires human confirmation for Draft rules', () => {
  const rule = findFailureRule('Baja presión de lubricante');
  assert.equal(requiresTechnicalValidation(rule), true);
  const state = createPrimeDemoState();
  const failureCase = createOrUpdateFailureCase([], state.stage, { ...failureInput('7720'), failureReason: rule!.FailureReason, ruleId: rule!.RuleId, ruleStatus: 'Draft - technical validation required' }, 1, '2026-08-06T10:00:00.000Z');
...
test('calculates ActualMinutes from work timestamps', () => {
  assert.equal(calculateActualMinutes('2026-08-06T10:00:00.000Z', '2026-08-06T10:12:00.000Z'), 12);
  assert.equal(calculateActualMinutes('2026-08-06T10:12:00.000Z', '2026-08-06T10:00:00.000Z'), null);
});
...
test('preserves exact 52-column export order', () => {
  assert.equal(PRIME_HEADERS.length, 52);
  assert.equal(PRIME_HEADERS[0], 'RecordId');
  assert.equal(PRIME_HEADERS[51], 'OffsetWellPressure');
});
